...
CREATE TABLE `techhouse`.`apply` (
  ...
  `prod_selectedEmail` VARCHAR(100)  default "없음",  							-- 당첨자 이메일
...
insert into board(b_writer, b_title, b_views, b_recommed, b_commentcount, b_text, b_uploadimg, b_category) values("b_writer","b_title",0,0,2,"b_text","b_uploadimg","자유게시판");
insert into board(b_writer, b_title, b_views, b_recommed, b_commentcount, b_text, b_uploadimg, b_category) values("b_writer","b_title",0,0,5,"b_text","b_uploadimg","자유게시판");
insert into board(b_writer, b_title, b_views, b_recommed, b_commentcount, b_text, b_uploadimg, b_category) values("b_writer","b_title",0,0,5,"b_text","b_uploadimg","인증게시판");
insert into board(b_writer, b_title, b_views, b_recommed, b_commentcount, b_text, b_uploadimg, b_category) values("b_writer","b_title",0,0,5,"b_text","b_uploadimg","인증게시판");
insert into board(b_writer, b_title, b_views, b_recommed, b_commentcount, b_text, b_uploadimg, b_category) values("b_writer","b_title",0,0,5,"b_text","b_uploadimg","취미공유");
insert into board(b_writer, b_title, b_views, b_recommed, b_commentcount, b_text, b_uploadimg, b_category) values("b_writer","b_title",0,0,5,"b_text","b_uploadimg","취미공유");

insert into apply(a_productName,a_applyEmail, a_applyName,a_applyTel ) values("a_productName","a_applyEmail","a_applyName","a_applyTel");
insert into apply(a_productName, a_applyEmail, a_applyName,a_applyTel) values("a_productName","a_applyEmail","a_applyName","a_applyTel");

insert into product(prod_productName, prod_price, prod_uploadimg, prod_selectedEmail, prod_selectedname, prod_selectedTel) values("GeForce RTX 4090 24GB GDDR6X
						Graphics Card","prod_price","prod_uploadimg","prod_selectedEmail","prod_selectedname","prod_selectedTel");
insert into product(prod_productName, prod_price, prod_uploadimg) values("감자 두 박스","300000","prod_uploadimg");

insert into `comment`(c_writer, c_text, c_boardSeq) values("c_writer","c_text1",100001);
insert into `comment`(c_writer, c_text, c_boardSeq) values("c_writer","c_text1",100001);
